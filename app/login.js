...
var Inv = {
  ...
  toFindPWD() {
    const {navigator} = pro;
    if (navigator) {
      navigator.push({name: 'findpwd', component: FindPWD})
    }
  }
};
  ...
  render() {
    return (
      <View style={{
        ...
        <View style={styles.container}>
          ...
          <View style={{
            ...
            <TextInput style={styles._intextinput} placeholder="请输入用户名或手机号码" underlineColorAndroid='transparent' onChangeText={(text) => {
                pros.loginName = text;
            ...
            <View style={{
              ...
              <TextInput style={styles._intextinput} placeholder="请输入密码" underlineColorAndroid='transparent' onChangeText={(text) => {
                  pros.passWord = text;
      ...
      _regist() { //TODO
        const {navigator} = this.props;
        if (navigator) {
          navigator.push({name: 'regist', component: Regist})
        }
      }
      ...
      _login() {
        ...
        * 调用原生方法
        */
        // NativeModules.DesUtil.encryptDES('1234','gxhlqn88',(s)=>{
        //   Alert.alert(s);
        // });
        // return;
        ...
        var url = Global.BASEURL + 'app/appUser!login.do';
        fetch(url, {
          method: 'POST',
          headers: {
          ...
          body: 'loginName=' + pros.loginName + '&passWord=' + pros.passWord + '&provinceId=1024'
